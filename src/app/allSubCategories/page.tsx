...
export default async function AllSubCategories() {
  let response = await fetch('https://ecommerce.routemisr.com/api/v1/subcategories',{
    method: 'GET',
    next: {revalidate: 30}
  }
  );
  let {data : allProducts}:{data : subCategorieItem[]} = await response.json();
  return (
    <>
    <div className="grid md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-5 mt-5">
      {allProducts.map((SubCategorie)=><SubCategoriesCard key={SubCategorie._id} SubCategorie={SubCategorie}/>) }
    </div>
    </>
  );
}
